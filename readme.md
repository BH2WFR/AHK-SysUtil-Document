...
5. `鼠标前进/后退键`：也叫 `XButton1` 和 `XButton2`，或 `鼠标测键`，是指一些多功能鼠标大拇指处多出的两个按键，默认按下时可触发 `前进/后退` 功能。
...
​	`RAlt Mode 1` ![](./assets/MainIcon.png)中，`RAlt+I/J/K/L` 可以以**较快的速度移动鼠标指针**，如需要像素级地慢速移动鼠标指针，应加入 `Shift` 键
...
​	在其中，`I/J/K/L/方向键` 或 `NumPad 8/4/2/6` 可以**像素级慢速移动鼠标指针**，`a + I/J/K/L/方向键` 或 `NumPad0 + NumPad 8/4/2/6/方向键` 可**快速移动鼠标指针**。
...
​	`RAlt + Left/Right` 或  部分多功能鼠标中的 `前进/后退 ` 键：已经将其适配为 `Alt+Left/Right` 等实现 **前进/后退**的快捷键，不同程序具体实现方式不同，未来将为更多的程序适配 前进/后退 功能。
...
![image-20231230224455000](./assets/image-20231230224455000.png)
...
![image-20231230224523475](./assets/image-20231230224523475.png)
...
<img src="./assets/image-20231115203339215.png" alt="image-20231115203339215" style="zoom:67%;" />
